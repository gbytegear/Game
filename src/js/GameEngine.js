      ...
      Object.defineProperties(this, {
        item: {
          ...
          set: value => {
            containdeItem = value;
            this.itemChanged();
            if (this.type) GameEngine.player.equipment[this.type] = this.item;
          }
        },
        selected: {
          get: () => selected,
  ...
  class Character {
    ...
    giveItem(item) {
      for (let i = 0; i < this.inventory.length; i++)
        if (this.inventory[i].name == 'empty') return this.inventory[i] = item;
    }
  }
  ...
  class MapController {
    ...
    hitSolid(obj) {
      ...
      for (let i = 0; i < this.solidObjects.children.length; i++)
        if (hitTest(this.solidObjects.children[i], obj)) return true;
      return false;
    }

    hitAction(obj) {
      if (this.actionArea.length == 0) return;
      this.actionArea.forEach( area => {
        if(hitTest(area, obj))area.fx();
      });
    }
  ...
  window.GameEngine = new class GameEngine {
    constructor() {
      ...
      mainController.canvas.rerenderChangeTimeout(canvas => {
        canvas.insert(this.map.tiles);
        canvas.insert(this.map.backgroundDecorations);
        canvas.insert(this.map.solidObjects);
        canvas.insert(this.player.rootElement);
        canvas.insert(this.map.foregroundDecorations);
      });
      ...
      this.mainLoop.insertFunction(() => mainController.canvas.render());
    ...
    movement() {
      ...
      if (this.controllerButtons.shift) speed += this.player.runningSpeed;
      if (this.controllerButtons.up) addY -= speed;
      if (this.controllerButtons.right) addX += speed;
      if (this.controllerButtons.down) addY += speed;
      if (this.controllerButtons.left) addX -= speed;
    ...
    intiControllEvents() {
      document.addEventListener('keydown', e => {
        if (e.keyCode === 38 /* up */ || e.keyCode === 87 /* w */ || e.keyCode === 38) this.controllerButtons.up = true;
        if (e.keyCode === 39 /* right */ || e.keyCode === 68 /* d */ || e.keyCode === 39) this.controllerButtons.right = true;
        if (e.keyCode === 40 /* down */ || e.keyCode === 83 /* s */ || e.keyCode === 40) this.controllerButtons.down = true;
        if (e.keyCode === 37 /* left */ || e.keyCode === 65 /* a */ || e.keyCode === 37) this.controllerButtons.left = true;
        if (e.keyCode === 16) this.controllerButtons.shift = true;

        if (e.keyCode === 27) {
          let ui = document.querySelector('.menu');
          window.running = ui.classList.contains('show');
          (ui.classList.contains('show')) ? (ui.classList.remove('show'), this.mainLoop.block = false) : (ui.classList.add('show'), this.mainLoop.block = true);
        }
      });


      document.addEventListener('keyup', e => {
        if (e.keyCode === 38 /* up */ || e.keyCode === 87 /* w */ || e.keyCode === 38) this.controllerButtons.up = false;
        if (e.keyCode === 39 /* right */ || e.keyCode === 68 /* d */ || e.keyCode === 39) this.controllerButtons.right = false;
        if (e.keyCode === 40 /* down */ || e.keyCode === 83 /* s */ || e.keyCode === 40) this.controllerButtons.down = false;
        if (e.keyCode === 37 /* left */ || e.keyCode === 65 /* a */ || e.keyCode === 37) this.controllerButtons.left = false;
        if (e.keyCode === 16) this.controllerButtons.shift = false;
      });
      ...
      document.addEventListener('mousedown', e => {
        if (this.mainLoop.block || !this.player.stats.armed) return;
        let mouse = {
        ...
        this.creationStack.push({ to: "background", object: bullet });
      });
    }
